...
import (
	...
	"github.com/arulrajnet/htpasswd-forward-auth/pkg/watcher"
...
func NewHTPasswdValidator(path string) (Validator, error) {
	...
	if err := watcher.WatchFileForUpdates(path, nil, func() {
		...
		if err != nil {
			logger.Error().Msgf("%v: no changes were made to the current htpasswd map", err)
		}
	}); err != nil {
...
func (h *htpasswdMap) loadHTPasswdFile(filename string) error {
	...
	defer func(c io.Closer) {
		cerr := c.Close()
		if cerr != nil {
			logger.Fatal().Msgf("error closing the htpasswd file: %v", cerr)
		}
	}(r)
